...
from ops import compute_spatial_encodings, binary_focal_loss,quality_focal_loss,OHEM_loss
from ops import GHMC
...
class InteractionHead(Module):
    ...
    def preprocess(self,
        ...
        for b_idx, detection in enumerate(detections):
            ...
            #update: keep  maximum number of object detection class-wise
            keep_mask = torch.zeros_like(o_idx)
            keep_mask_final = []
            for class_id in torch.unique(o_idx):
                curr_indices = torch.where(o_idx == class_id, o_idx, 0)
                keep_mask = torch.nonzero(curr_indices)
                if keep_mask.size()[0] >= self.max_object:
                    keep_mask = keep_mask[:self.max_object]
                for i in keep_mask.view(-1).tolist():
                    keep_mask_final.append(i)
            torch.tensor(keep_mask_final)
            o_idx = o_idx[keep_mask_final]
            #print(keep_mask_final)
    ...
    def compute_interaction_classification_loss(self, results: List[dict]) -> Tensor:
        ...
        """loss = quality_focal_loss(
            scores, labels, gamma = 2, reduction='sum'
        )"""
        
        loss = OHEM_loss(
            scores, labels, rate = 0.75, 
        )
        """GHMC_loss = GHMC()
        loss = GHMC_loss(scores,labels,1)"""
        return loss
        #normalizing loss by postive logits when using binary_focal_loss
        #return loss / n_p
    ...
    def compute_interactiveness_loss(self, results: List[dict]) -> Tensor:
        ...
        #)
        """loss = quality_focal_loss(
            weights, labels, gamma = 2, reduction='sum'
        )"""
        loss = OHEM_loss(
            weights, labels, rate = 0.75, 
        )
        """GHMC_loss = GHMC()
        loss = GHMC_loss(weights,labels,1)"""
        return loss
...
class MultiBranchFusion(Module):
    ...
    def forward(self, appearance: Tensor, spatial: Tensor) -> Tensor:
        return F.relu(torch.stack([
            ...
            #fusion_weight
            #fc_3(F.relu(self.fusion_weight * fc_1(appearance) * fc_2(spatial) + (1 - self.fusion_weight) * (fc_1(appearance) + fc_2(spatial))))
            for fc_1, fc_2, fc_3
...
class MessageMBF(MultiBranchFusion):
    ...
    def _forward_human_nodes(self, appearance: Tensor, spatial: Tensor) -> Tensor:
        ...
        return torch.stack([
            fc_3(F.relu(
                fc_1(appearance).repeat(n, 1, 1) * fc_2(spatial).permute([1, 0, 2])
                #fusion_weight
                #(self.fusion_weight * fc_1(appearance).repeat(n, 1, 1) * fc_2(spatial).permute([1, 0, 2]) 
                #+ (1 - self.fusion_weight) * (fc_1(appearance).repeat(n, 1, 1) + fc_2(spatial).permute([1, 0, 2])))
    ...
    def _forward_object_nodes(self, appearance: Tensor, spatial: Tensor) -> Tensor:
        ...
        return torch.stack([
            fc_3(F.relu(
                ...
                * fc_2(spatial)
                #fusion_weight
                #(self.fusion_weight * fc_1(appearance).repeat(n_h, 1, 1) * fc_2(spatial)
                #+ (1 - self.fusion_weight) * (fc_1(appearance).repeat(n_h, 1, 1) + fc_2(spatial)))
            )) for fc_1, fc_2, fc_3 in zip(self.fc_1, self.fc_2, self.fc_3)
        ]).sum(dim=0)
...
class GraphHead(Module):
    ...
    def compute_prior_scores(self,
        ...
        -----------
            ...
            scores: Tensor[N]
                Object detection scores ( a a pairing)
