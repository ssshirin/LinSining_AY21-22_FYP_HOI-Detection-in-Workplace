...
from pocket.models import fasterrcnn_resnet_fpn,fasterrcnn_mobilenet_v3_large_fpn
...
def main(args):
	...
	dataset = HICODet(
        root=os.path.join(args.data_root,
            "hifo_data/images/{}".format(args.partition)),
	...
	if args.net == 'resnet':
		...
		detector = fasterrcnn_mobilenet_v3_large_fpn('mobilenetv3', pretrained=True, box_score_thresh=args.score_thresh, 
			box_nms_thresh=args.nms_thresh,
			box_detections_per_img=args.num_detections_per_image,
			num_classes=89,trainable_backbone_layers=4)
