...
class DataFactory(Dataset):
    def __init__(self,
            ...
            flip=False,
            smallestMaxSize=False,
            perspective=False,
            box_score_thresh_h=0.2,
            box_score_thresh_o=0.2
            ):
        if name not in ['hicodet', 'vcoco','hifo']:
        ...
        if name == 'hicodet':
            ...
            assert partition in ['train', 'test'], \
                "Unknown HIFO partition " + partition
            self.dataset = HICODet(
                root=os.path.join(data_root, 'hifo_data/images', partition),
                anno_file=os.path.join(data_root, 'instances_{}.json'.format(partition)),
                target_transform=pocket.ops.ToTensor(input_format='dict')
            )
        elif name == 'vcoco':
        ...
        self._smallestrMaxSize = torch.randint(0, 2, (len(self.dataset),)) if smallestMaxSize \
            else torch.zeros(len(self.dataset))
        self._perspective = torch.randint(0, 2, (len(self.dataset),)) if perspective \
            else torch.zeros(len(self.dataset))
    ...
    def smallestMaxSize_boxes(self, detection, target):
        detection['boxes'] = SmallestMaxSize.apply_to_bboxes(detection['boxes'], max_size=640)
        target['boxes_h'] = SmallestMaxSize.apply_to_bboxes(target['boxes_h'],max_size=640)
        target['boxes_o'] = SmallestMaxSize.apply_to_bboxes(target['boxes_o'],max_size=640)
    
    def perspective_boxes(self, detection,target):
        detection['boxes'] = Perspective.apply_to_bboxes(detection['boxes'])
        target['boxes_h'] = Perspective.apply_to_bboxes(target['boxes_h'])
        target['boxes_o'] = Perspective.apply_to_bboxes(target['boxes_o'])
    

    def __getitem__(self, i):
        image, target = self.dataset[i]
        if self.name == 'hicodet' or 'hifo':
        ...
        if self._smallestrMaxSize[i]:
            openCV_img = np.array(image)
            openCV_img = SmallestMaxSize.apply(openCV_img, max_size=640)
            self.smallestMaxSize_boxes(detection, target)
            image = Image.fromarray(openCV_img)
        
        if self._perspective[i]:
            openCV_img = np.array(image)
            p=Perspective()
            openCV_img = p.apply(openCV_img)
            self.perspective_boxes(detection, target)
            image = Image.fromarray(openCV_img)
...
def test(net, test_loader):
    ...
    #update detectionAPMeter number of gt
...
class CustomisedDLE(DistributedLearningEngine):
    def __init__(self, net, train_loader, val_loader, num_classes=117, **kwargs):
        ...
        #for history plot
        self.history_hoi_loss = []
        self.history_intr_loss = []
    ...
    def _on_end_epoch(self):
        ...
        if self._rank == 0:
            """self.history_train.append(ap_train.mean().item())
            self.history_val.append(ap_val.mean().item())"""
            #print(ap_train)
            #print(ap_val)
            #keep only the used verb classes
            keep_idx=[36,41,56,70,71,114,117,118,119]
            ap_train_tf=[]
            ap_val_tf=[]
            for i in keep_idx:
                ap_train_tf.append(ap_train[i])
                ap_val_tf.append(ap_val[i])
            ap_train_tf=torch.stack(ap_train_tf)
            ap_val_tf=torch.stack(ap_val_tf)
            print(ap_train_tf,ap_val_tf)
            #ap_train=ap_train[36,41,56,70,71,114,117,118,119]
            #ap_val=ap_val[36,41,56,70,71,114,117,118,119]
            print("Epoch: {} | training mAP: {:.4f}, evaluation time: {:.2f}s |"
                "validation mAP: {:.4f}, total time: {:.2f}s\n".format(
                    self._state.epoch, 
                    #ap_train.mean().item(), 
                    ap_train_tf.mean().item(), 
                    timer[0],
                    #ap_val.mean().item(),
                    ap_val_tf.mean().item(), 
                    timer[1]
    ...
    def _print_statistics(self):
        ...
        if self._rank == 0:
            """self.history_hoi_loss.append(hoi_loss)
            self.history_intr_loss.append(intr_loss)"""
